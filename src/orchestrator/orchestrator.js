...
import { DragTool } from '../annotation/input/dragTool.js';
import { TextEntryManager } from '../annotation/input/textEntryManager.js';
...
export class Orchestrator {
    constructor({ extensionPath, settings, gradiaSettings, selectionClearer, getPortalMode, openPreferences }) {
        ...
        this._gradiaSettings = gradiaSettings;
        this._selectionClearer = selectionClearer;
        this._getPortalMode = getPortalMode ?? (() => false);
    ...
    _setupTooling(primaryBin) {
        ...
        this._screenshotCapture.portalMode = this.portalMode;
        ...
        this._ocrSelector = new OcrSelector({
            ...
            extensionPath: this._extensionPath,
            screenshotFn: async () => {
                const file = await this._screenshotCapture.capture({ ocr: true, portalMode: this.portalMode });
    ...
    capture(opts) {
        return this._screenshotCapture?.capture(opts);
    }
