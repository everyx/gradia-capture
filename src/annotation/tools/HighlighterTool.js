...
export class HighlighterTool extends DrawingTool {
    ...
    get name() {
        return N_('Highlighter');
    }
    get icon() {
        return 'icons/marker-symbolic.svg';
    }
    get keybindings() {
        return [Clutter.KEY_6, Clutter.KEY_section, Clutter.KEY_h];
    }
    get propSchema() {
        return [
            { key: 'color', type: 's', default: '#ffdd00' },
            { key: 'size', type: 'd', default: 4 },
        ];
    }
    bounds(s) {
        return rectBounds(s.stagePoints, SELECTION_PADDING + (s.strokeWidth ?? 4) * 4);
    }
    hitTest(s, x, y) {
