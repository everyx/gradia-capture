...
- `annotation` 不得反向 import `capture` / `utilities` / `ui`(只通过 `getToolDef().render` 回调 + `getMenuItems()` 纯数据暴露渲染/菜单)。
...
见 [`docs/adr/`](docs/adr/)。当前：

- [0001](docs/adr/0001-capturecontext-stays-injectable-function.md)：`captureContext` 保持普通可注入函数，不形式化为 port（单 adapter + 无测试，形式化属过度设计）。
