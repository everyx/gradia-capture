...
| **annotation** | `src/annotation/` | 标注工具声明与渲染。`tools/` 下工具 def(`DrawingTool` 基类 + 9 工具含 `BlurTool`),共享领域能力提至 `annotation/shared.js`。`BlurTool` 的像素化后端引擎作其内部实现置于 `tools/blur/engine.js`——blur 是 tool,引擎是其实现细节,不与之平级。 |
...
| **interaction** | `src/interaction/` | 输入→命令翻译器:`dragTool`、`textEntryManager`。把用户输入翻译成对画板/组件的命令,不认识具体工具(经 `stroke.hitBounds` 等能力)。 |
| **ui** | `src/ui/` | GNOME 原生 UI 适配,分 `adapters/`(工具栏/菜单/分辨率层/选区清理等原生适配)与 `widgets/`(展示构件 `squareSlider`)。 |
...
`shortcutDispatcher`(`src/orchestrator/shortcutDispatcher.js`,属 `orchestrator`)只做**键码 → 意图**映射(`KEY_MAP` + `_matchIntent` + 读 `TOOL_SHORTCUTS` 的 `_matchToolShortcut`),执行委托给 `orchestrator.execute(intent)`。工具选自身键已在 `annotation/tools/index.js` 的 `keybindings` 声明,dispatcher 不碰工具逻辑。会话级命令(undo/copy/save-as/ocr-*)由 `orchestrator.execute` 翻译成对各 context 已有接口的调用。
